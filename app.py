...
class Note(db.Model):
    ...
    is_public = db.Column(db.Boolean, default=False) # new!
...
#new!
# Assuming you're using Flask and SQLAlchemy for your model

@app.route('/explore', methods=['GET'])
def explore():
    # Get the topic filter from the request (if provided)
    topic_filter = request.args.get('topic', '')

    if topic_filter:
        # If a topic filter is provided, filter notes by that topic
        notes = Note.query.filter(Note.topic.ilike(f'%{topic_filter}%')).all()
    else:
        # If no topic filter, return all notes
        notes = Note.query.all()

    return render_template('explore.html', notes=notes, topic_filter=topic_filter)


@app.route('/api/notes/public', methods=['GET'])
@login_required
def get_public_notes():
    notes = Note.query.filter_by(is_public=True).order_by(Note.id.desc()).limit(20).all()
    return jsonify([
        {'title': n.title, 'content': n.content, 'user_id': n.user_id}
        for n in notes
    ])

@app.route('/api/notes/<int:note_id>/vote', methods=['POST'])
@login_required
